 ...
 */
export function providerFieldPath(providerId: string, field: string): string[] {
 ...
 * @returns the settings path.
 */
export function modelFieldPath(providerId: string, modelIndex: number, field: string): string[] {
  return ['providers', providerId, 'models', String(modelIndex), field]
}

/** Path to the plugin's global-header mapping. */
export const GLOBAL_HEADERS_PATH: readonly string[] = ['globalHeaders']

/** Path to the plugin's durable UI preferences. */
export const PLUGIN_UI_PATH: readonly string[] = ['ui']
